...
WebHDFS.prototype._sendRequest = function _sendRequest (method, url, opts, callback) {
  ...
  function onComplete(err, res, body)
  {
    ...
    if ((res && self._isError(res)) || (err && err.code === 'ECONNREFUSED' && err.port != 50075)) // don't retry if datanode is down
    {
      if(
            self._opts.host.length > 1 &&
            ((body && body.RemoteException && body.RemoteException.exception === 'StandbyException')||
             (err && err.code === 'ECONNREFUSED' && err.port != 50075))
        )
      {
        if(!opts._retry)
        {
          ...
          console.log('[webhdfs #sendRequest]: Retrying for url: ' + url);
...
WebHDFS.prototype.readFile = function readFile (path, opts, callback) {
  ...
  remoteFileStream.once('error', function onError (err) {
    if(err && err.code === 'ECONNREFUSED' && err.port !== 50075) error = err;
  });
...
WebHDFS.prototype.createWriteStream = function createWriteStream (path, append, opts) {
  ...
  var req = request(params, function (err, res, body) {
    ...
    if (err || self._isError(res)) {
      if(typeof err === 'object' && ('code' in err && err.code === 'ECONNREFUSED' && err.port != 50075 )) // don't retry if datanode is down
...
function _doWriteStreamRetry(self, _req, params, append, path, opts, stream){
    ...
    console.log('[webhdfs #dowriteStreamRetry]: Retrying with url: '+ params.url);
...
function _doRetry(self, req, params){
    ...
    download.on('error', function(err) {
      console.log(`[webhdfs]: Received error after retrying ${req._curr_retry_idx} times `, err);
      if(err.port != 50075)
      {
      ...
      {
        console.log('Time elapsed since first retry ', Date.now() - req._retry_start_ms );
        console.log('Time left until giving up on retry', self._dn_crash_retry_limit_min * 60 * 1e3 - Date.now() + req._retry_start_ms );
        if(Date.now() - req._retry_start_ms > (self._dn_crash_retry_limit_min * 60 * 1e3))
        {
            ...
            setTimeout( _doRetry, self._dn_crash_retry_interval_ms, self, req, params);
      }
...
WebHDFS.prototype.createReadStream = function createReadStream (path, opts) {
  ...
  req.on('error', function(error){
    if(self._opts.host.length > 1 && error.code === 'ECONNREFUSED')
    {
      if( error.port != 50075 ) // retry other NN if the error didn't come from datanode
      {
        self._curr_host_idx = !self._curr_host_idx + 0;
        self._url.hostname  = self._opts.host[self._curr_host_idx];
        params.url          = self._getOperationEndpoint('open', path, opts);
      }
      console.log('[webhdfs #createReadStream]: Retrying with url: '+ params.url);
      req.retrying = true;
